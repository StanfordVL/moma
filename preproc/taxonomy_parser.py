...
class TaxonomyParser:
    ...
    def dump(self, verbose=True):
        ...
        with open(
            os.path.join(self.dir_moma, "anns/taxonomy/intransitive_action.json"), "w"
        ) as f:
            json.dump(self.taxonomy_ia, f, indent=2, sort_keys=True)
        with open(
            os.path.join(self.dir_moma, "anns/taxonomy/transitive_action.json"), "w"
        ) as f:
            json.dump(self.taxonomy_ta, f, indent=2, sort_keys=True)
        with open(
            os.path.join(self.dir_moma, "anns/taxonomy/attribute.json"), "w"
        ) as f:
            json.dump(self.taxonomy_att, f, indent=2, sort_keys=True)
        with open(
            os.path.join(self.dir_moma, "anns/taxonomy/relationship.json"), "w"
        ) as f:
