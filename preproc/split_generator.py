...
class SplitGenerator:
  def __init__(self, dir_moma):
    ...
    self.cnames_sact = [[sact['class_name'] for sact in ann['activity']['sub_activities']] for ann in anns]
  ...
  def generate_standard_splits(self, ratio):
    ...
    for i in range(10000):
      ...
      cnames_sact_train = set(itertools.chain(*[self.cnames_sact[j] for j in indices_train]))
      cnames_sact_val = set(itertools.chain(*[self.cnames_sact[j] for j in indices_val]))
      cnames_sact_test = set(itertools.chain(*[self.cnames_sact[j] for j in indices_test]))

      if len(cnames_act_train) == len(cnames_act_val) == len(cnames_act_test) and \
         len(cnames_sact_train) == len(cnames_sact_val) == len(cnames_sact_test):
        break
