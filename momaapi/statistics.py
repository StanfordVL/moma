    ...
    def _get_statistics(self, paradigm=None, split=None):
        ...
        if paradigm is None and split is None:
            ...
            ids_sact = list(
                itertools.chain(
                    *[
                        self.lookup.map_id("ids_sact", id_act=id_act)
                        for id_act in ids_act
                    ]
                )
            )
            anns_sact = [
                self.lookup.retrieve("ann_sact", id_sact) for id_sact in ids_sact
            ]
            ids_hoi = list(
                itertools.chain(
                    *[
        ...
        bincount_act = np.bincount(
            [ann_act.cid for ann_act in anns_act], minlength=len(self.taxonomy["act"])
        ).tolist()
        bincount_sact = np.bincount(
            [ann_sact.cid for ann_sact in anns_sact],
            minlength=len(self.taxonomy["sact"]),
        ).tolist()
        (
            bincount_actor,
            bincount_object,
        ...
        bincount_actor = np.bincount(
            bincount_actor, minlength=len(self.taxonomy["actor"])
        ).tolist()
        bincount_object = np.bincount(
            bincount_object, minlength=len(self.taxonomy["object"])
        ).tolist()
        bincount_ia = np.bincount(
            bincount_ia, minlength=len(self.taxonomy["ia"])
        ).tolist()
        bincount_ta = np.bincount(
            bincount_ta, minlength=len(self.taxonomy["ta"])
        ).tolist()
        bincount_att = np.bincount(
            bincount_att, minlength=len(self.taxonomy["att"])
        ).tolist()
        bincount_rel = np.bincount(
            bincount_rel, minlength=len(self.taxonomy["rel"])
        ).tolist()
